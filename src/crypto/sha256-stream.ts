...
export class Sha256Stream {
    ...
    update(bytes: Uint8Array): this {
        if (this.result) throw new Error('SHA-256 stream is finalized');
    ...
    digest(): Uint8Array {
        ...
        for (let index = 0; index < this.state.length; index++) output.setUint32(index * 4, this.state[index]);
    ...
    private compress(bytes: Uint8Array): void {
        ...
        for (let index = 0; index < 16; index++) this.words[index] = input.getUint32(index * 4);
        ...
        for (let index = 0; index < 64; index++) {
            ...
            const temp1 = (h + upperE + choose + ROUND[index] + this.words[index]) >>> 0;
